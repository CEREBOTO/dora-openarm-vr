...
# v1 gripper: converts joint space (meters, 0~0.044) ↔ motor space (radians, 0~-1.0472)
_GRIPPER_JOINT_MAX = 0.044       # open position in meters
_GRIPPER_MOTOR_MAX = -1.0472     # open position in radians (=-60°)


def _meter_to_motor_radians(joint_meters: float) -> float:
    """Convert gripper joint meters (0=closed, 0.044=open) to motor radians."""
    return (joint_meters / _GRIPPER_JOINT_MAX) * _GRIPPER_MOTOR_MAX


def _motor_radians_to_meter(motor_radians: float) -> float:
    """Convert motor radians back to gripper joint meters."""
    return _GRIPPER_JOINT_MAX * (motor_radians / _GRIPPER_MOTOR_MAX)
...
class SingleArmDriver:
    ...
    def set_latest_state(self, timeout_us=300):
        ...
        qpos = list(qpos)
        # v1: convert gripper motor radians → joint meters for consistent state
        if self.gripper_posforce:
            qpos[-1] = _motor_radians_to_meter(qpos[-1])
        self.latest_state = {
    ...
    def send_position(self, position: ArrayLike):
        ...
        if self.gripper_posforce:
            # Convert gripper from joint meters → motor radians for POS_FORCE
            gripper_motor_rad = _meter_to_motor_radians(target_pos[-1])
